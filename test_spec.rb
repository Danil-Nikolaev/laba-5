...
RSpec.describe LaboratoryOne do
  TestObject = LaboratoryOne.new
  describe '#part_one' do
    it 'return -2 for x ==0' do
      expect(TestObject.part_one(0)).to eq(-2)
    end
    it 'return true random number' do
      array_number = (1..100).map { Random.rand(-30..10) }
      array_number_bool = false
      array_number_bool = true if array_number.select { |num| TestObject.part_one(num).nil? }.empty?
    ...
    it 'return false random string' do
      ...
      array_string_bool = true if array_string.split.reject { |num| TestObject.part_one(num).nil? }.empty?
      expect(array_string_bool).to eq(true)
    end
    it 'return nil for x == 5**(1/3)' do
      number_bool = false
      number_bool = true if TestObject.part_one(5**(1.0 / 3)).nil?
  ...
  describe '#part_two' do
    it 'return true random number' do
      ...
      array_number_bool = false
      array_number_bool = true if array_number.select { |num| TestObject.part_two(num).nil? }.empty?
      expect(array_number_bool).to eq(true)
    end
    it 'return false random float number' do
      array_number = (1..100).map { Random.rand(1.0..30.0) }
      array_number_bool = false
      array_number_bool = true if array_number.reject { |num| TestObject.part_two(num).nil? }.empty?
      expect(array_number_bool).to eq(true)
    end
    it 'return false random string' do
      array_string = Random.rand(22..51).times.map { (0...(rand(10))).map { ('a'..'z').to_a[rand(26)] }.join }.join(' ')
      array_string_bool = false
      array_string_bool = true if array_string.split.reject { |num| TestObject.part_two(num).nil? }.empty?
      expect(array_string_bool).to eq(true)
    end
  end
  describe '#part_three' do
    it 'check for random string' do
      # str_array = Array.new(20) do
      #   Random.rand(22..51).times.map do
      #     (0...(rand(10))).map do
      #       ('a'..'z').to_a[rand(26)]
      #     end.join
      #   end.join(' ')
      ...
      str_array_sort = []
      check_para = true
      str_array = TestObject.part_three str_array
      str_array.each do |i|
        i_arr = i.split
